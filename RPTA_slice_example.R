...
#### (Main artcle) rank R=1, normal noise add, random slice outliers ####
m = 100
n = p = 64
...
# replace random horizontal slices with outlying slices
out.mode = 3 # select mode direction (1,2,3) that is contaminated with outliers
nslice.out = 1 # number of outlying slices: 1,3,5

if (out.mode == 1) {
  indx.out = sample(1:m, nslice.out, replace=FALSE)
  for (i in indx.out) {
    X.out[i,,] = runif(n*p, min=max(X.sig), max=1.5*max(X.sig))
  }
} else if (out.mode == 2) {
  indx.out = sample(1:n, nslice.out, replace=FALSE)
  for (j in indx.out) {
    X.out[,j,] = runif(m*p, min=max(X.sig), max=1.5*max(X.sig))
  }
} else {
  indx.out = sample(1:p, nslice.out, replace=FALSE)
  for (k in indx.out) {
    X.out[,,k] = runif(m*n, min=max(X.sig), max=1.5*max(X.sig))
  }
}
data = X.out
...
#### M2: RCP (L2 loss, P1) ####
lambdaRange = list(u=c(1e-8, 1e4), v=c(1e-8, 1e4), w=c(1e-8, 1e4))
nlambda = list(u=24, v=24, w=24)
rcp.out = RobRSVD_tnsr(data=data, irobust=FALSE, inits=init_s, initu=init_u, initv=init_v, initw=init_w, lambdaRange = lambdaRange, degree=3,
...
lambdaRange = list(u=c(1e-8, 1e4), v=c(1e-8, 1e4), w=c(1e-8, 1e4))
nlambda = list(u=24, v=24, w=24)
robrcp2.out = RobRSVD2_tnsr(data=data, irobust=TRUE, inits=init_s, initu=init_u, initv=init_v, initw=init_w, lambdaRange = lambdaRange, degree=3,
                            nlambda=nlambda, iugcv=TRUE, ivgcv=TRUE, iwgcv=TRUE, isspline=FALSE, verbose = TRUE, niter=20)
...
#### (Appendix) rank R=2, normal noise add, random slice outliers ####
m = 100
n = p = 64
...
if (out.mode == 1) {
  indx.out = sample(1:m, nslice.out, replace=FALSE)
  for (i in indx.out) {
    X.out[i,,] = X.out[i,,] + runif(n*p, min=1*max(X.sig), max=1.5*max(X.sig))
  }
} else if (out.mode == 2) {
  indx.out = sample(1:n, nslice.out, replace=FALSE)
  for (j in indx.out) {
    X.out[,j,] = X.out[,j,] + runif(m*p, min=1*max(X.sig), max=1.5*max(X.sig))
  }
} else {
  indx.out = sample(1:p, nslice.out, replace=FALSE)
  for (k in indx.out) {
    X.out[,,k] = X.out[,,k] + runif(m*n, min=1*max(X.sig), max=1.5*max(X.sig))
  }
}
